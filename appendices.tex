...
For the parallel implementation with multiprocessing this approach wasn't supported. They gave some ideas to try but to no avail. Linking extrae with a number of different libraries, such as the pthreads one, to see it they were able to hook themselves to the python multiprocessing threads didn't work. The issue was left open. Fortunately, a workaround was found: the command line usage of extrae was used. It has two basic commands:
...
\item [Paraver and other analysis tools,] \hfill \\ to ease the understanding process with execution diagrams. Also, on this first stage, we will start looking for the best available tools for the posterior analysis stage.
